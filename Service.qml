...
Item {
  ...
  function reconcileSettings() {
    ...
    var previous = JSON.stringify(_settings)
    _settings = checked.value
    _inlineEntry = _clone(entry || { id: moduleName })
    _settingsError = null
    if (_initializationComplete && previous !== JSON.stringify(_settings)) _evaluateSchedule(true)
    _maybeInitialize()
    return true
  }
  ...
  function updateSettings(changes) {
    ...
    // Publish locally before persistence.  shellConfigChanged then reconciles
    // the authoritative resulting transaction.
    _inlineEntry = merged
    _settings = checked.value
    _settingsError = null
    if (shell && typeof shell.updateEntryInline === "function")
      shell.updateEntryInline(moduleName, merged)
    _evaluateSchedule(true)
    return true
  }

  function setAutomationEnabled(value) { return updateSettings({ automationEnabled: value }) }
  function setNightTemperature(value) { return updateSettings({ nightTemperature: value }) }
  function setTransitionMinutes(value) { return updateSettings({ transitionMinutes: value }) }
  ...
  function _handleBackendStatus(message, pending) {
    ...
    if (_initializationComplete && _backendAvailable && _backendProbed) {
      ...
      else if (!_override && _scheduleValid && _settings.automationEnabled) _sendScheduled(false)
    }
  }
  ...
  function _evaluateSchedule(forceApply, resumeOverride, observational) {
  ...
  function _sendScheduled(force, resume) {
    ...
    // proves the display is at its target.  Resume deliberately bypasses this:
    // it must reach Controller.py to clear an override even when its pixels do
    // not need changing.  Likewise, force may replace an in-flight divergent
    // target, but it never turns a matching state into a write.
    if (resume !== true && _sameTarget(_actual, desired)) {
      _lastSent = marker
      return false
    }
    ...
    _lastSent = marker
    _applyBusy = true
    var fields = { desired: desired, intent: "schedule", overrideUntil: _schedule.nextBoundaryMs }
